  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-panda-line/80 glass">
      <div className="mx-auto grid max-w-5xl grid-cols-4 px-3 pb-[calc(0.55rem+env(safe-area-inset-bottom))] pt-2">
        {items.map((item) => (
          <Link key={item.href} href={item.href} prefetch className="flex flex-col items-center gap-1 rounded-2xl py-1.5 text-xs font-medium text-panda-muted active:bg-panda-paper">
